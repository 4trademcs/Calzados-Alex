...
export default function ProductCard({
  ...
  const src = url || image || "/placeholder.svg";

  const displayName = useMemo(() => {
    const pretty = String(name ?? "").replace(/[-_]+/g, " ").trim().replace(/\s+/g, " ");
  ...
  const MATERIALS = ["tela", "charol"];

  const COLORS_BY_MATERIAL = {
    tela: ["Negro", "Verde oscuro", "Verde Azúl", "Mostaza", "Rosa vieja", "Mamonsillo", "Beich", "Gris", "Carmelita", "Rojo"],
    charol: ["Negro", "Blanco", "Beich"],
  };

  const COLOR_HEX = {
    Negro: "#000000", "Verde oscuro": "#006400", "Verde Azúl": "#0E7490", Mostaza: "#D4A017",
    "Rosa vieja": "#C08081", Mamonsillo: "#B9C400", Beich: "#D9C3A3", Gris: "#7A7A7A",
    Carmelita: "#8B5A2B", Rojo: "#C00000", Blanco: "#FFFFFF",
  };

  const [tipo, setTipo] = useState(PRICE_BY_TYPE[initialType] ? initialType : "medio");
  const [material, setMaterial] = useState(MATERIALS[0]); // solo por select
  const [colorName, setColorName] = useState(COLORS_BY_MATERIAL[MATERIALS[0]][0]);
  ...
  const whatsappMessage = useMemo(() => {
    const msg = `Hola quiero hacer mi pedido del modelo: ${modelVal} de tipo: ${tipo} y color: ${colorName} del material: ${material}.`;
    ...
    <Tooltip content={`Material actual: ${material}`} position="top">
      ...
      >
        ...
        <div className="basicInfo relative rounded-[25px] bg-[#f5f5f5] p-5 pb-10 select-none" >
          ...
          {/* Tipo + Material (solo por selects) */}
          <div
          ...
          >
            ...
            <div className="flex flex-col items-start gap-2 z-10">
              <label htmlFor={`tipo-${id}`} className="text-sm font-bold text-zinc-700">
                Tipo:
              </label>
              <select
                id={`tipo-${id}`}
                ...
                "
              >
                <option value="bajo">tacon bajo</option>
                <option value="medio">tacon medio</option>
                <option value="alto">tacon alto</option>
            ...
            <div className="flex flex-col items-end gap-2 z-10">
              <label htmlFor={`material-${id}`} className="text-sm font-semibold text-zinc-700">
                Material:
              </label>
              <select
                id={`material-${id}`}
          ...
          <div className="addCard absolute bottom-3 left-0 w-full">
            ...
            <button
              type="button"
              onClick={(e) => { e.stopPropagation(); setShowColorModal(true); }}
            ...
            <Link
              ...
              onClick={(e) => {
                e.stopPropagation();
                onShoeClick({ id, name: modelVal, src, tipo, color: colorName, material, price });
              }}
          ...
          >
            <div className="mb-4">
              ...
              <p className="text-sm text-zinc-600">
                Toca un color para seleccionarlo. Cambia el material desde el selector superior.
              </p>
            </div>
          ...
          >
            <div className="mb-4">
              ...
              <p className="text-sm text-zinc-600">
                ...
                Modelo:<span className="text-black font-bold"> {modelVal} </span>
                de tipo:<span className="text-black font-bold"> {tipo} </span>
                y color:<span className="text-black font-bold"> {colorName} </span>
                del material:<span className="text-black font-bold"> {material} </span>.
                Estás seguro?
              </p>
            </div>
            <button
